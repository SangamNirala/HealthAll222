#!/usr/bin/env python3
"""
🧠 COMPREHENSIVE BACKEND TESTING: STEP 1 REVOLUTIONARY ADAPTIVE LEARNING & PERSONALIZATION ENGINE
================================================================================================

Comprehensive testing of the newly implemented Step 1 Adaptive Learning & Personalization Engine
...
- Learning feedback loops with conversation outcomes
- Privacy-compliant population learning with basic anonymization
- Real-time performance optimization <15ms target
- Integration with existing medical AI system
- MongoDB collections (patient_learning_profiles, conversation_learning_data, population_learning_patterns)

PERFORMANCE TARGETS:
- Processing time <15ms for adaptive learning operations
- 99.9% data consistency in learning profiles
- Successful integration with existing Quick Health Tracking chatbot
...
# Test configuration
BACKEND_URL = os.getenv('REACT_APP_BACKEND_URL', 'https://multi-symptom-engine.preview.emergentagent.com')
API_BASE = f"{BACKEND_URL}/api"

class AdaptiveLearningTester:
    """Comprehensive tester for Step 1 Adaptive Learning & Personalization Engine"""
    ...
    async def test_update_patient_profile_endpoint(self):
    ...
    async def test_patient_insights_endpoint(self):
    ...
    async def test_feedback_integration_endpoint(self):
        ...
        feedback_scenarios = [
            ...
            {
                "name": "Complex Multi-Intent Learning Feedback",
                "patient_id": self.test_patient_ids[2],
                "conversation_id": f"conv_{uuid.uuid4().hex[:8]}",
                "feedback_data": {
                ...
                "outcome_data": {
                    ...
                    "intent_prioritization_correct": True,
                    "conversation_completed": True,
                    "specialist_referral_appropriate": True,
                    "processing_efficiency": 0.86
                }
            }
        ]
    ...
    async def test_learning_analytics_endpoint(self):
            ...
            if response.status_code == 200:
                ...
                if not missing_fields:
                    ...
                    if overview_complete and metrics_complete and insights_valid:
                        ...
                        performance_valid = (
                            avg_processing_time >= 0 and
                            isinstance(performance_target_achieved, bool) and
                            system_overview.get('total_learning_profiles', 0) >= 0
                        )
                        
                        self.log_test_result(
                            "Learning Analytics - Performance Validation",
                            performance_valid,
                            f"Performance metrics validation. Processing time valid: {avg_processing_time >= 0}, "
                            f"Target status valid: {isinstance(performance_target_achieved, bool)}, "
                            f"Profile count valid: {system_overview.get('total_learning_profiles', 0) >= 0}",
                            0
                        )
    ...
    async def test_performance_targets(self):
        ...
        print("⚡ TESTING: Performance Targets & Optimization")
        print("-" * 50)
        
        # Test processing time targets
        processing_times = [metric['processing_time_ms'] for metric in self.performance_metrics if metric['processing_time_ms'] > 0]
        ...
        if processing_times:
            ...
            # Check <15ms target
            target_met_count = sum(1 for time_ms in processing_times if time_ms < 15.0)
            target_met_percentage = (target_met_count / len(processing_times)) * 100
            
            self.log_test_result(
                "Performance Target - <15ms Processing",
                target_met_percentage >= 80,  # 80% should meet target
                f"Processing time analysis. Avg: {avg_processing_time:.2f}ms, "
                f"Min: {min_processing_time:.2f}ms, Max: {max_processing_time:.2f}ms, "
                f"Target met: {target_met_percentage:.1f}% of tests",
                avg_processing_time
            )
        else:
            self.log_test_result(
    ...
    async def run_comprehensive_tests(self):
        ...
        # Run all test suites
        await self.test_update_patient_profile_endpoint()
        await self.test_patient_insights_endpoint()
        await self.test_feedback_integration_endpoint()
        await self.test_learning_analytics_endpoint()
    ...
    def generate_test_report(self):
        ...
        endpoint_groups = {
            ...
            "Performance & Integration": [r for r in self.test_results if any(keyword in r['test_name'] for keyword in ["Performance", "Integration"])]
        }
        ...
        # Critical issues
        critical_failures = [r for r in self.test_results if not r['success'] and any(keyword in r['test_name'] for keyword in ["Update Patient Profile", "Performance Target"])]
        ...
        # Success highlights
        key_successes = [r for r in self.test_results if r['success'] and any(keyword in r['test_name'] for keyword in ["Update Patient Profile", "Learning Analytics", "Performance Target"])]
